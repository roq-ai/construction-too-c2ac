...
import { getCustomerProfiles } from 'apiSdk/customer-profiles';
...
function DashboardPage() {
  ...
  const customerProfileStat = useSWR<PaginatedInterface<CustomerProfileInterface>>(
    () => `/customer-profiles`,
    () => getCustomerProfiles({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: customerProfileStat?.data?.totalCount,
      isLoading: customerProfileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
